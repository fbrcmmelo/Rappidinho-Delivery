...
public class PedidoDAO implements GenericDAO {
    ...
    public List<Object> listar() {
        ...
        String sql = "select p.*,pro.id_produto,pro.nome_produto,c.id_contratado,pes.nome_pessoa, r.id_rota, r.nome_rota from pedido p, produto pro,\n"
                ...
                + "p.id_produto = pro.id_produto and pes.id_pessoa = c.id_pessoa order by p.data_pedido desc;";// nome da tabela do banco (TUDOMINUSCULO)
        ...
        try {
            ...
            while (rs.next()) {
                ...
                pedido.setPessoa(new Pessoa(rs.getInt("id_pessoa"), rs.getString("nome_pessoa")));
    ...
    public Object carregar(int idObject) {
        ...
        try {
            ...
            while (rs.next()) {
                oPedido = new Pedido();
                oPedido.setIdPedido(rs.getInt("id_pedido"));
                oPedido.setDataPedido(rs.getDate("data_pedido"));
                oPedido.setLatDestinoPedido(rs.getDouble("lat_destino_pedido"));
                oPedido.setLngDestinoPedido(rs.getDouble("lng_destino_pedido"));
                oPedido.setPessoa(new Pessoa(rs.getInt("id_pedido")));
            }

        } catch (Exception ex) {
            System.out.println("Problemas ao Carregar Pedido " + ex.getMessage());
            ex.printStackTrace();
        } finally {
            try {
                ConnectionFactory.closeConnection(conn, stmt, rs);
    ...
    public Boolean alterar(Object object) {
        ...
        String sql = "update pedido set lat_destino_pedido=?, lng_destino_pedido=?, status_pedido=?, id_produto=?, "
                + "id_contratado=?, id_rota=? "
                + "where id_pedido =?";

        try {
            stmt = conn.prepareStatement(sql);
            stmt.setDouble(1, oPedido.getLatDestinoPedido());
            stmt.setDouble(2, oPedido.getLngDestinoPedido());
            stmt.setString(3, oPedido.getStatusPedido());
            stmt.setInt(4, oPedido.getProduto().getIdProduto());
            stmt.setInt(5, oPedido.getContratado().getIdContratado());
            stmt.setInt(6, oPedido.getRota().getIdRota());
            stmt.setInt(7, oPedido.getIdPedido());
            stmt.execute();
            return true;
