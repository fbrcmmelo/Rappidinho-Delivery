...
public class EntregaDAO implements GenericDAO {
    ...
    public Boolean cadastrar(Object object) {
        ...
        String sql = "insert into entrega (data_entrega, status_entrega, id_pessoa, id_pedido) values (?, ?, ?, ?);";

        try {
            stmt = conn.prepareStatement(sql);
            stmt.setDate(1, new java.sql.Date(oEntrega.getDataEntrega().getTime()));
            stmt.setString(2, oEntrega.getStatusEntrega());
            stmt.setInt(3, oEntrega.getPessoa().getIdPessoa());
    ...
    public List<Object> listar() {
        ...
        PreparedStatement stmt = null;
        ResultSet rs = null;
        String sql = "select e.*,p.data_pedido,p.id_pedido,pes.id_pessoa,pes.nome_pessoa from entrega e ,"
                + " pedido p, pessoa pes where e.id_pedido = p.id_pedido and e.id_pessoa = pes.id_pessoa"
                + " order by p.data_pedido desc;";// nome da tabela do banco (TUDOMINUSCULO)
        ...
        try {
            ...
            while (rs.next()) {
                ...
                entrega.setPedido(new Pedido(rs.getInt("id_pedido"), rs.getDate("data_pedido")));
                entrega.setPessoa(new Pessoa(rs.getInt("id_pessoa"), rs.getString("nome_pessoa")));
